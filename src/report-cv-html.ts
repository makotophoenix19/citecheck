...
export function renderCvHtml(a: CvAnalysis, sourceName: string, generatedAt = new Date()): string {
  ...
  const tiles = [
    ...
    a.profile.firstAuthor ? tile(String(a.profile.firstAuthor), "first-author") : "",
    a.profile.corresponding ? tile(String(a.profile.corresponding), "corresponding-author") : "",
  ...
  const dupBlock = a.profile.duplicates.length
    ? `<h2>Possible duplicates · ${a.profile.duplicates.length}</h2>
  <p class="info">The same title is listed more than once — often an abstract and its full article. Worth confirming these aren't inflating the count.</p>
  <table><tbody>${a.profile.duplicates.slice(0, 10).map((d) => `<tr><td class="st" style="color:var(--warn)">${d.count}×</td><td>${esc(d.title)}</td></tr>`).join("")}</tbody></table>`
    : "";
